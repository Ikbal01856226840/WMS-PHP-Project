                                    ...
                                    <div class="col-xl-6 col-md-6 col-sm-6 col-xs-6" >
                                        <div class="card-block ">
                                            ...
                                            <div class="form-group my-2 my-2 row">
                                                    ...
                                                    <div class="col-md-8">
                                                        <span id="batch_no_span" class="" style="font-size: small;"></span>
                                                        <input type="text" name="BatchNo" id="batch_no" class="form-control duplicate_batch_no barcode" placeholder="" required>
                                                    </div>
                                              </div>
                                        </div>
                                    </div>
...
<script>
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if (data == '0') {
                    ...
                    $("#user_name_span").text(' Battery Type ' + product_name + " Available").addClass("badge badge-success");
                    $(":submit").removeAttr("disabled");
                } else {
                    $("#user_name_span").removeClass("badge badge-success");
                    $("#user_name_span").text(' Battery Type ' + product_name + " Already Exists").addClass("badge badge-danger");
    ...
    $('.duplicate_batch_no').on('keyup', function() {
        var batch_no = $('#batch_no').val().toUpperCase();
        $.ajax({
            url: "<?php echo route('duplicate_batch_no/check')?>",
            type: "GET",
            data: {
                "batch_no": batch_no
            },
            ...
            success: function(data) {
                console.log(data);
                if (data == '0') {
                    $("#batch_no_span").removeClass("badge badge-danger");
                    $("#batch_no_span").text(' Batch No ' + batch_no + " Available").addClass("badge badge-success");
                    $(":submit").removeAttr("disabled");
                } else {
                    $("#batch_no_span").removeClass("badge badge-success");
                    $("#batch_no_span").text(' Batch No ' + batch_no + " Already Exists").addClass("badge badge-danger");
                    $(":submit").attr("disabled", true);
                }
            }
        });
