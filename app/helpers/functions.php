...
if(!function_exists('multi_select')){
    function multi_select($name='',$data=[],$title='',$select=[],$attributes='',$class=''){
        $value='';
        $select1=[];
        $selected='';
        if(!empty($select)){
            for($i=0;$i<count($select);$i++){
                $select1[$select[$i]]= $select[$i];
            }
            if($select[0]==0) $selected='selected';
        }

        if(!empty($data)){
            $value="<select multiple='multiple' name='".$name."[]' class='select2 form-control select_multiple select2-hidden-accessible select_order ".$class."' style='width:100%;' ".$attributes.">";
            $value.="<option ".$selected." value='0'>All</option>";
            $selected='';
            foreach($data as $row){
                if(array_key_exists($row['id'], $select1)){
                    $selected='selected';
                }else{
                    $selected=''; 
                }
                $value .="<option ".$selected." value='".$row['id']."'>".$row[$title]."</option>";
